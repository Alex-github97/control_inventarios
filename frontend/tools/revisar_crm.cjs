...
const RUTAS = [
  ['/crm', 'Tablero'], ['/crm/clientes', 'Clientes'], ['/crm/leads', 'Prospectos'],
  ['/crm/oportunidades', 'Oportunidades'], ['/crm/cotizaciones', 'Cotizaciones'],
  ['/crm/contratos', 'Contratos'], ['/crm/tickets', 'Tickets'],
  ['/crm/interacciones', 'Interacciones'], ['/crm/campanas', 'Campanas'],
  ['/crm/encuestas', 'Encuestas'], ['/crm/cuentas-clave', 'Cuentas clave'],
  ['/crm/rentabilidad', 'Rentabilidad'], ['/crm/ia', 'Senales'],
  ['/crm/reportes', 'Reportes'], ['/crm/config', 'Configuracion'],
];
...
(async () => {
  ...
  for (const [ruta, nombre] of RUTAS) {
    ...
    p.on('response', r => {
      if (r.url().includes('/api/v1/crm/')) {
        respuestas.push(`${r.status()} ${r.url().split('/crm/')[1].split('?')[0]}`);
      }
    });
    ...
    try {
      ...
      // Cero peticiones al CRM significa que la pantalla nunca se monto —el
      ...
      await p.screenshot({
        path: `${SALIDA}/crm-${ruta.replace(/\//g, '_')}.png`, fullPage: false,
      });
    } catch (e) {
      fallos++;
