      ...
      <Box sx={{ p:3, background:PAGE_BG, minHeight:'100vh' }}>
        <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:3 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
            ...
            <Box><Typography variant="h5" sx={{ fontWeight:800, color:'#FFF', lineHeight:1 }}>Gestión de Auditorías</Typography><Typography sx={{ fontSize:12, color:LBL }}>GRC · Auditorías internas, externas y seguimientos</Typography></Box>
          ...
          <Box sx={{ flex:1, minWidth:0 }}>
            <Paper sx={{ bgcolor:'transparent', overflowX:'auto' }}>
              <Table size="small">
                ...
                <TableBody>
                  {filtered.map(a=>(
                    <TableRow key={a.id} onClick={()=>setSel(a)} sx={{ cursor:'pointer', bgcolor:sel?.id===a.id?alpha(GRC_COLOR,.06):'transparent', '&:hover':{ bgcolor:alpha(GRC_COLOR,.04) }, '& td':{ borderColor:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.8)', fontSize:12 } }}>
                      <TableCell sx={{ fontSize:10.5, color:LBL }}>{a.id}</TableCell>
                      <TableCell sx={{ maxWidth:220 }}><Typography sx={{ fontSize:12.5, color:'#FFF', fontWeight:600, lineHeight:1.3 }}>{a.titulo}</Typography></TableCell>
                      ...
                      <TableCell sx={{ minWidth:100 }}><Box sx={{ display:'flex', alignItems:'center', gap:1 }}><LinearProgress variant="determinate" value={a.avance} sx={{ flex:1, height:5, borderRadius:2.5, bgcolor:'rgba(255,255,255,0.08)', '& .MuiLinearProgress-bar':{ bgcolor:ESTADO_COLOR[a.estado] } }} /><Typography sx={{ fontSize:11, fontWeight:700, color:ESTADO_COLOR[a.estado], minWidth:28 }}>{a.avance}%</Typography></Box></TableCell>
          ...
          {sel&&(
            <Box sx={{ width:370, flexShrink:0, bgcolor:CARD_BG, border:`1px solid ${CARD_BOR}`, borderRadius:2, p:2.5, height:'fit-content' }}>
              ...
              {sel.hallazgos.length>0&&<>
                ...
                {sel.hallazgos.map((h,i)=><Box key={i} sx={{ display:'flex', gap:1, mb:.75, alignItems:'flex-start' }}><Box sx={{ width:5, height:5, borderRadius:'50%', bgcolor:'#D97706', mt:.75, flexShrink:0 }} /><Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.65)' }}>{h}</Typography></Box>)}
              </>}
              <Divider sx={{ borderColor:'rgba(255,255,255,0.06)', my:1.5 }} />
              ...
              {sel.informe ? (
                <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', p:.75, bgcolor:'rgba(255,255,255,0.03)', borderRadius:1, border:'1px solid rgba(255,255,255,0.06)', mb:1 }}>
                  <Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.7)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1, mr:1 }}>{sel.informe}</Typography>
                  <IconButton size="small" onClick={()=>handleDownload(sel.informe!)} sx={{ color:GRC_COLOR, p:.5 }}><FileDownload sx={{ fontSize:14 }} /></IconButton>
                </Box>
              ) : (
                <Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.35)', mb:1 }}>Sin informe cargado</Typography>
              )}
        ...
        <Dialog open={dlgOpen} onClose={()=>setDlgOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx:{ bgcolor:'#1F2937', color:'#FFF' } }}>
          ...
          <DialogContent sx={{ display:'flex', flexDirection:'column', gap:2, pt:'16px !important' }}>
            ...
            <Box sx={{ display:'flex', gap:2 }}>
              <TextField label="Fecha Inicio" type="date" fullWidth size="small" value={form.inicio} onChange={sf('inicio')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
              <TextField label="Fecha Fin" type="date" fullWidth size="small" value={form.fin} onChange={sf('fin')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
            </Box>
