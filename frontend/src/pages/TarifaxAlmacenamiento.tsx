...
import {
  ...
  Bolt, Groups, Construction, Devices, RestartAlt, Tune, Edit as EditIcon, Close, BookmarkAdded,
...
// ─── Cálculo (mismo modelo que el backend, con rubros de la plataforma) ────────
...
export default function TarifaxAlmacenamiento() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setTemplate(cf.data); setPlataformas(pl.data || [])
      if (pl.data?.length) cargarDraft(pl.data[0])
  ...
  const guardar = async () => {
    ...
    setGuardando(true)
    try {
      const saved = await persistir(draft)
      const lista = (await apiClient.get<Plataforma[]>('/tarifax/plataformas')).data || []
      setPlataformas(lista); cargarDraft(saved)
      toast.success('Plataforma guardada')
    } catch (e: any) { toast.error(e.response?.data?.detail || 'No se pudo guardar') }
    finally { setGuardando(false) }
  }

  // Diálogo (crear / editar identidad + capacidades + áreas)
  const abrirNueva = () => { if (!template) return; setDlgMode('new'); setDlgData(plataformaNueva(template)); setDlgOpen(true) }
  ...
  const guardarDialogo = async () => {
    ...
    setGuardando(true)
    try {
      const saved = await persistir(dlgData)
      const lista = (await apiClient.get<Plataforma[]>('/tarifax/plataformas')).data || []
      setPlataformas(lista); cargarDraft(saved); setDlgOpen(false)
      toast.success(dlgMode === 'new' ? 'Plataforma creada' : 'Plataforma actualizada')
    } catch (e: any) { toast.error(e.response?.data?.detail || 'No se pudo guardar') }
    finally { setGuardando(false) }
  }
  const eliminar = async () => {
    ...
    try {
      await apiClient.delete(`/tarifax/plataformas/${draft.id}`)
      const lista = (await apiClient.get<Plataforma[]>('/tarifax/plataformas')).data || []
      setPlataformas(lista)
      if (lista.length) cargarDraft(lista[0]); else { setDraft(null); setSelId(null) }
  ...
  const guardarComoPlantilla = async () => {
    if (!draft) return
    if (!window.confirm('¿Usar los rubros e inputs de esta plataforma como plantilla por defecto para nuevas plataformas?')) return
    try {
      ...
      await apiClient.put('/tarifax/cotizacion-config', tmpl); setTemplate(clone(tmpl))
      toast.success('Guardado como plantilla por defecto')
  ...
  const exportar = async (tipo: 'excel' | 'pdf') => {
    ...
    if (dirty && !window.confirm('Tienes cambios sin guardar. ¿Exportar con los valores actuales de todos modos?')) return
    setExportando(tipo)
    try {
    ...
    <Layout title="TarifaX · Cotización de Almacenamiento">
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2.5, flexWrap: 'wrap' }}>
        ...
        <Box sx={{ flex: 1, minWidth: 200 }}>
          ...
          <Typography variant="body2" sx={{ color: '#64748B', fontSize: 12 }}>Rubros configurables por plataforma · valor por posición · cotización formal en PDF</Typography>
        </Box>
      ...
      {plataformas.length === 0 && (
        <Card sx={{ p: 4, textAlign: 'center', border: `2px dashed ${alpha(TX_COLOR, 0.35)}`, borderRadius: '14px', mb: 2 }}>
          ...
          <Typography sx={{ fontSize: 13, color: '#64748B', mb: 2 }}>Crea tu primera bodega/plataforma para empezar a cotizar. Vendrá con los rubros pre-configurados y podrás editarlos.</Typography>
      ...
      {draft && r && (
        <Box sx={{ display: 'flex', gap: 2.5, flexDirection: { xs: 'column', lg: 'row' }, alignItems: 'flex-start' }}>
            ...
            <Card sx={{ p: 2.25, mb: 2, border: `1px solid ${alpha(TX_COLOR, 0.2)}`, borderRadius: '14px' }}>
              ...
              <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
                <Button size="small" fullWidth variant="outlined" startIcon={<EditIcon />} onClick={abrirEditar}
                  sx={{ color: '#475569', borderColor: '#CBD5E1', textTransform: 'none', fontWeight: 700, borderRadius: '8px' }}>Datos y áreas</Button>
              ...
              <Grid container spacing={1}>
                {[
                  ...
                  { l: 'Posiciones', v: fmt(num(draft.capacidad_posiciones)) },
                ].map(x => (
          ...
          <Box sx={{ flex: 1, minWidth: 0, width: '100%' }}>
            {/* Inputs / parámetros de la plataforma */}
            <Card sx={{ p: 2.25, mb: 2, border: `1px solid ${alpha(TX_COLOR, 0.2)}`, borderRadius: '14px' }}>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5, flexWrap: 'wrap' }}>
                ...
                <Button size="small" variant="text" startIcon={<BookmarkAdded />} onClick={guardarComoPlantilla} sx={{ color: '#64748B', textTransform: 'none', fontWeight: 700 }}>Fijar plantilla</Button>
              ...
              <Grid container spacing={1.25}>
                ...
                <Grid item xs={6} sm={4} md={3}>
                  <Box sx={{ p: 1, borderRadius: '8px', bgcolor: alpha(TX_COLOR, 0.06), height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <Typography sx={{ fontSize: 14, fontWeight: 800, color: TX_DARK }}>{fmt(num(draft.parametros.pallet_largo_m) * num(draft.parametros.pallet_ancho_m), 2)} m²</Typography>
                  </Box>
                </Grid>
              </Grid>
            </Card>
            ...
            {/* ARRIENDO — áreas editables (por plataforma) */}
            <Card sx={{ p: 2.25, mb: 2, border: '1px solid #E5E7EB', borderRadius: '14px' }}>
              ...
              <Typography sx={{ fontSize: 11.5, color: '#64748B', mb: 1 }}>
                Utilizados: <b>{fmt(r.m2Util)} m²</b> ({fmt(r.pctUtil * 100, 1)}%) · Valor/m²: <b>{money(r.valorM2)}</b> (arriendo ÷ m² totales).
              </Typography>
              <Box sx={{ overflowX: 'auto' }}>
                <Table size="small" sx={{ minWidth: 460 }}>
                  ...
                  <TableBody>
                    {draft.areas.map((a, i) => (
                      <TableRow key={i}>
                        <TableCell sx={CELL_SX}><TxtCell value={a.area} width={240} onChange={v => setD({ areas: updRow(draft.areas, i, { area: v }) })} /></TableCell>
                        <TableCell sx={{ ...CELL_SX, textAlign: 'right' }}><NumCell value={a.m2} width={80} onChange={v => setD({ areas: updRow(draft.areas, i, { m2: v }) })} /></TableCell>
                        ...
                        <TableCell sx={{ ...CELL_SX, textAlign: 'right', px: 0 }}><IconButton size="small" onClick={() => setD({ areas: draft.areas.filter((_, k) => k !== i) })} sx={{ color: '#CBD5E1', '&:hover': { color: '#DC2626' } }}><DeleteOutline sx={{ fontSize: 16 }} /></IconButton></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              </Box>
              <Button size="small" startIcon={<Add />} onClick={() => setD({ areas: [...draft.areas, { area: '', m2: 0 }] })}
      ...
      {/* ── Diálogo: crear / editar identidad, capacidades y áreas ── */}
      {dlgData && (
        <Dialog open={dlgOpen} onClose={() => setDlgOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: '16px' } }}>
          <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1.5, pb: 1 }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontSize: 16, fontWeight: 800, color: '#1E293B' }}>{dlgMode === 'new' ? 'Nueva plataforma' : 'Datos de la plataforma'}</Typography>
          ...
          <DialogContent dividers sx={{ bgcolor: '#FCFCFD' }}>
            ...
            <Grid container spacing={1.5}>
              ...
              <Grid item xs={12} sm={4}><TextField size="small" label="Posición / referencia" placeholder="Bodega 4, zona franca…" value={dlgData.posicion} onChange={e => setDlgData({ ...dlgData, posicion: e.target.value })} fullWidth /></Grid>
            </Grid>
            ...
            {(() => {
              ...
              const chips = [
                ...
                { l: '% ocupación', v: `${fmt(pct * 100, 1)}%` },
                { l: 'M² / posición', v: num(dlgData.capacidad_posiciones) ? fmt(m2u / num(dlgData.capacidad_posiciones), 2) : '—' },
              ]
              return (
            ...
            {dlgMode === 'new' && <Typography sx={{ fontSize: 11.5, color: '#94A3B8', mt: 1.5 }}>Los rubros (nómina, servicios, maquinaria, equipos) se copiarán desde la plantilla por defecto y podrás editarlos por plataforma.</Typography>}
          </DialogContent>
