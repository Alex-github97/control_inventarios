    ...
    <Layout>
      <Box sx={{ p: 3, bgcolor: DARK_BG, minHeight: '100vh' }}>
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#FFF' }}>Configuración CRM</Typography>
        ...
        {tab === 0 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2 }}>Parámetros y Umbrales del Sistema</Typography>
            {UMBRALES.map((u, i) => (
              <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.75, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <Box>
                  <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{u.param}</Typography>
                  <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', mt: 0.25 }}>Rango: {u.min} – {u.max} {u.tipo}</Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Box sx={{ px: 2, py: 0.75, bgcolor: alpha(CRM_COLOR, 0.12), border: `1px solid ${alpha(CRM_COLOR, 0.25)}`, borderRadius: 1, minWidth: 70, textAlign: 'center' }}>
                    ...
                    <Typography sx={{ fontSize: 9.5, color: 'rgba(255,255,255,0.35)' }}>{u.tipo}</Typography>
                  </Box>
                </Box>
              </Box>
            ))}
        ...
        {tab === 1 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2 }}>Configuración de Alertas y Notificaciones</Typography>
            {NOTIFICACIONES.map((n, i) => (
              <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.75, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <Box>
                  <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{n.tipo}</Typography>
                  <Chip label={n.canal} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)', fontSize: 10, mt: 0.75 }} />
                </Box>
                <Switch checked={notifs[i]} onChange={e => setNotifs(prev => { const c = [...prev]; c[i] = e.target.checked; return c })}
                  sx={{ '& .MuiSwitch-thumb': { bgcolor: notifs[i] ? CRM_COLOR : '#4B5563' }, '& .MuiSwitch-track': { bgcolor: notifs[i] ? alpha(CRM_COLOR, 0.35) : 'rgba(255,255,255,0.1)' } }} />
              </Box>
            ))}
          </Box>
        )}
        ...
        {tab === 2 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 0.5 }}>Modelo de Lead Scoring IA</Typography>
            <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', mb: 2.5 }}>Suma de pesos = 100 · Umbral CALIENTE: ≥75 · TIBIO: 50-74 · FRÍO: &lt;50</Typography>
            {LEAD_SCORING.map((f, i) => (
              <Box key={i} sx={{ mb: 2.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75 }}>
                  <Typography sx={{ fontSize: 12.5, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>{f.factor}</Typography>
          ...
          <Grid container spacing={2}>
            {INTEGRACIONES.map((int, i) => (
              <Grid key={i} size={{ xs: 12, sm: 6, md: 4 }}>
                <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${alpha(integs[i] ? int.color : '#4B5563', 0.3)}`, borderRadius: 2, p: 2 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
                    ...
                    <Switch checked={integs[i]} onChange={e => setIntegs(prev => { const c = [...prev]; c[i] = e.target.checked; return c })}
                      size="small"
                      sx={{ '& .MuiSwitch-thumb': { bgcolor: integs[i] ? int.color : '#4B5563' }, '& .MuiSwitch-track': { bgcolor: integs[i] ? alpha(int.color, 0.35) : 'rgba(255,255,255,0.1)' } }} />
                  </Box>
                  <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>{int.desc}</Typography>
                  <Chip label={integs[i] ? 'Conectado' : 'Desconectado'} size="small"
                    sx={{ mt: 1.5, bgcolor: integs[i] ? alpha('#059669', 0.12) : alpha('#4B5563', 0.2), color: integs[i] ? '#059669' : '#6B7280', fontSize: 10 }} />
                </Box>
              </Grid>
            ))}
          </Grid>
