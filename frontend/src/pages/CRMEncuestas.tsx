    ...
    <Layout>
      <Box sx={{ p: 3, bgcolor: DARK_BG, minHeight: '100vh' }}>
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#FFF' }}>Customer Experience</Typography>
          ...
          {[
            ...
            <Grid key={i} size={{ xs: 6, md: 3 }}>
              <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${alpha(k.color, 0.3)}`, borderRadius: 2, p: 2 }}>
                ...
                <Typography sx={{ fontSize: 10.5, color: 'rgba(255,255,255,0.35)', mt: 0.25 }}>{k.sub}</Typography>
              </Box>
            </Grid>
          ))}
        </Grid>
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 5 }}>
              <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
                <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2 }}>NPS — Distribución</Typography>
                <Box sx={{ textAlign: 'center', mb: 3 }}>
                  <Typography sx={{ fontSize: 56, fontWeight: 900, color: '#059669', lineHeight: 1 }}>+{NPS_SCORE}</Typography>
                  <Typography sx={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', mt: 0.5 }}>Net Promoter Score</Typography>
                </Box>
                {[
                  ...
                  <Box key={i} sx={{ mb: 2 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                      <Typography sx={{ fontSize: 12.5, color: 'rgba(255,255,255,0.7)' }}>{s.label}</Typography>
            ...
            <Grid size={{ xs: 12, md: 7 }}>
              <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
                <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2 }}>Comentarios Recientes</Typography>
                {ENCUESTAS.filter(e => e.respondida && e.comentario).slice(0, 5).map((e, i) => {
                  const cfg = TIPO_CFG[e.tipo]
                  return (
                    <Box key={i} sx={{ mb: 1.5, p: 1.5, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1.5 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75, flexWrap: 'wrap', gap: 1 }}>
                        <Typography sx={{ fontSize: 12, fontWeight: 600, color: '#FFF' }}>{e.cliente}</Typography>
                      ...
                      <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', fontStyle: 'italic' }}>"{e.comentario}"</Typography>
                      <Typography sx={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', mt: 0.5 }}>{e.fecha}</Typography>
                    </Box>
                  )
                })}
              </Box>
                  ...
                  {ENCUESTAS.map((e, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        ...
                        <td style={{ padding: '10px 14px', fontSize: 12.5, color: '#FFF', fontWeight: 600, whiteSpace: 'nowrap' }}>{e.cliente}</td>
                        ...
                        <td style={{ padding: '10px 14px' }}>
                          {e.puntaje !== null ? <NPSBadge score={e.puntaje} /> : <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>Pendiente</Typography>}
                        </td>
                        <td style={{ padding: '10px 14px' }}>
                        ...
                        <td style={{ padding: '10px 14px', fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>{e.fecha}</td>
                        <td style={{ padding: '10px 14px', fontSize: 11.5, color: 'rgba(255,255,255,0.55)', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.comentario || '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        {tab === 2 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2.5 }}>Áreas de Mejora — Plan de Acción 2026</Typography>
            {MEJORAS.map((m, i) => {
              ...
              return (
                <Box key={i} sx={{ mb: 2.5 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75 }}>
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{m.area}</Typography>
                      <Chip label={m.prioridad} size="small" sx={{ bgcolor: alpha(pcol, 0.15), color: pcol, fontSize: 9.5, fontWeight: 700 }} />
                    </Box>
                    <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                      <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Actual: {m.puntaje}</Typography>
                      <Typography sx={{ fontSize: 12, color: '#059669', fontWeight: 700 }}>Meta: {m.meta}</Typography>
                    </Box>
                  </Box>
                  <Box sx={{ height: 10, borderRadius: 5, bgcolor: 'rgba(255,255,255,0.07)', overflow: 'hidden', position: 'relative' }}>
