    ...
    <Layout>
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
          ...
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 800, color: 'text.primary', lineHeight: 1 }}>GRC Inteligencia Artificial</Typography>
            <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>GRC · Predicciones · Anomalías · Recomendaciones IA</Typography>
          </Box>
        ...
        <TabPanel value={tab} index={0}>
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {INSIGHTS.map(ins => (
              <Grid key={ins.titulo} size={{ xs: 12, md: 6 }}>
                <Card sx={{ border: `1px solid ${alpha(ins.color, 0.3)}`, borderRadius: 2 }}>
                  <CardContent>
                    ...
                    <Typography sx={{ fontWeight: 700, color: 'text.primary', fontSize: 14, mb: 1, lineHeight: 1.3 }}>{ins.titulo}</Typography>
                    <Typography sx={{ fontSize: 12, color: 'text.secondary', lineHeight: 1.5, mb: 1.5 }}>{ins.descripcion}</Typography>
                    <LinearProgress variant="determinate" value={ins.confianza} sx={{ height: 3, borderRadius: 2, bgcolor: 'text.disabled', mb: 1.5, '& .MuiLinearProgress-bar': { bgcolor: ins.color } }} />
          ...
          <Card sx={{ border: `1px solid ${alpha(AI_COLOR, 0.3)}`, borderRadius: 2 }}>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                <Psychology sx={{ color: AI_COLOR, fontSize: 18 }} />
                <Typography sx={{ fontWeight: 700, color: 'text.primary', fontSize: 14 }}>Recomendaciones Estratégicas IA</Typography>
              </Box>
              {RECOMENDACIONES.map((r, i) => (
                <Box key={i} sx={{ display: 'flex', gap: 1.5, mb: 1.25 }}>
                  ...
                  <Typography sx={{ fontSize: 12.5, color: 'text.primary', lineHeight: 1.4 }}>{r}</Typography>
                </Box>
              ))}
            </CardContent>
          </Card>
            ...
            {PREDICCIONES.map(p => (
              <Grid key={p.proceso} size={{ xs: 12, md: 6 }}>
                <Card sx={{ border: `1px solid #E5E7EB`, borderRadius: 2 }}>
                  <CardContent>
                    <Typography sx={{ fontWeight: 700, color: 'text.primary', fontSize: 14, mb: 1.5 }}>{p.proceso}</Typography>
                    <Box sx={{ mb: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>Prob. Incidente (90 días)</Typography>
                      ...
                      <LinearProgress variant="determinate" value={p.prob_incidente} sx={{ height: 5, borderRadius: 2, bgcolor: 'text.disabled', '& .MuiLinearProgress-bar': { bgcolor: p.prob_incidente >= 60 ? '#DC2626' : p.prob_incidente >= 40 ? '#D97706' : '#059669' } }} />
                    </Box>
                    <Box sx={{ mb: 1.5 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>Prob. Incumplimiento</Typography>
        ...
        <TabPanel value={tab} index={2}>
          <Card sx={{ border: `1px solid #E5E7EB`, borderRadius: 2 }}>
            <CardContent>
              <Typography sx={{ fontWeight: 700, color: 'text.primary', mb: 2, fontSize: 14 }}>Evolución de Riesgos 2026 (Ene–Jun)</Typography>
              <Box sx={{ display: 'flex', gap: 3, mb: 3 }}>
                {[{l:'Críticos',c:'#DC2626'},{l:'Altos',c:'#EA580C'},{l:'Medios',c:'#D97706'}].map(item => (
                  <Box key={item.l} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    ...
                    <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>{item.l}</Typography>
                  </Box>
                ))}
              </Box>
              <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'flex-end', height: 120 }}>
                {TENDENCIAS_RIESGOS.map(t => (
                  <Box key={t.mes} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.25 }}>
                    ...
                    <Typography sx={{ fontSize: 9, color: 'text.secondary', mt: 0.5 }}>{t.mes}</Typography>
                  </Box>
                ))}
              </Box>
