    ...
    <Layout>
      <Box sx={{ p: 3, bgcolor: DARK_BG, minHeight: '100vh' }}>
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#FFF' }}>Campañas Comerciales</Typography>
        ...
        {tab === 0 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            {CAMPANAS.map((c, i) => {
              ...
              return (
                <Box key={i} sx={{ bgcolor: CARD_BG, border: `1px solid ${alpha(cfg.color, 0.2)}`, borderRadius: 2, p: 2 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2, mb: 1.5 }}>
                    <Box>
                      ...
                      <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF' }}>{c.nombre}</Typography>
                      <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', mt: 0.25 }}>{c.inicio} → {c.fin}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                      {[
                        ...
                        <Box key={j} sx={{ textAlign: 'center' }}>
                          ...
                          <Typography sx={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>{m.label}</Typography>
                        </Box>
                      ))}
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', gap: 2, p: 1.5, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1.5 }}>
                    <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Presupuesto: <Box component="span" sx={{ color: '#FFF', fontWeight: 600 }}>${c.presupuesto}M</Box></Typography>
                  </Box>
                </Box>
              )
            })}
        ...
        {tab === 1 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2.5 }}>Métricas Email — Newsletter Retail Q3</Typography>
              ...
              return (
                <Box key={i} sx={{ mb: 2.5 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{m.semana}</Typography>
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Enviados: {m.enviados}</Typography>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <Box sx={{ flex: 1 }}>
                      <Typography sx={{ fontSize: 9.5, color: 'rgba(255,255,255,0.3)', mb: 0.5 }}>Tasa apertura</Typography>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Typography sx={{ fontSize: 9.5, color: 'rgba(255,255,255,0.3)', mb: 0.5 }}>Tasa clics</Typography>
                      <Box sx={{ height: 10, borderRadius: 5, bgcolor: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
