      ...
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 800, color: '#FFF', lineHeight: 1 }}>Gestión de Auditorías</Typography>
              <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>QMS · Auditorías internas y externas</Typography>
            </Box>
              ...
              <TableBody>
                {AUDITORIAS.map(a => (
                  <TableRow key={a.codigo} sx={{ '& td': { borderColor: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.7)', fontSize: 12 } }}>
                    ...
                    <TableCell>{a.resultado ? <Chip label={a.resultado} size="small" sx={{ fontSize: 9, height: 18, bgcolor: alpha(RES_COLOR[a.resultado], 0.15), color: RES_COLOR[a.resultado], fontWeight: 700 }} /> : <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>—</Typography>}</TableCell>
                    <TableCell sx={{ textAlign: 'center' }}><Chip label={a.hallazgos} size="small" sx={{ fontSize: 11, height: 20, bgcolor: a.hallazgos > 0 ? alpha('#DC2626', 0.15) : 'rgba(255,255,255,0.05)', color: a.hallazgos > 0 ? '#DC2626' : 'rgba(255,255,255,0.3)' }} /></TableCell>
        ...
        <TabPanel value={tab} index={2}>
          <Card sx={{ bgcolor: '#111827', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 2 }}>
            <CardContent>
              <Typography sx={{ fontWeight: 700, color: '#FFF', mb: 2 }}>Programa de Auditorías 2026</Typography>
              {/* Gantt simplificado */}
                ...
                <Box sx={{ minWidth: 700 }}>
                  ...
                  <Box sx={{ display: 'flex', mb: 1, ml: 18 }}>
                    {['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map(m => (
                      <Box key={m} sx={{ flex: 1, textAlign: 'center' }}><Typography sx={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', fontWeight: 700 }}>{m}</Typography></Box>
                    ))}
                  </Box>
                  {[
                    ...
                    <Box key={r.nombre} sx={{ display: 'flex', alignItems: 'center', mb: 1, height: 28 }}>
                      <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', width: 70, flexShrink: 0, mr: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.nombre}</Typography>
                      <Box sx={{ flex: 1, display: 'flex', position: 'relative', height: '100%' }}>
                        {Array.from({ length: 12 }).map((_, i) => (
                          <Box key={i} sx={{ flex: 1, height: '100%', bgcolor: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent', borderLeft: '1px solid rgba(255,255,255,0.04)' }} />
                        ))}
        ...
        <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#1F2937', color: '#FFF' } }}>
          ...
          <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField label="Fecha Inicio Plan" type="date" fullWidth size="small" InputLabelProps={{ shrink: true, sx: { color: 'rgba(255,255,255,0.5)' } }} sx={{ '& .MuiOutlinedInput-root': { color: '#FFF', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }} />
              </Grid>
            </Grid>
          </DialogContent>
