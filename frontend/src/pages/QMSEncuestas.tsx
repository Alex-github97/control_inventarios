      ...
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 800, color: '#FFF', lineHeight: 1 }}>Encuestas de Satisfacción</Typography>
              <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>QMS · NPS · CSAT · Clima organizacional</Typography>
            </Box>
            ...
            {ENCUESTAS.map(e => {
              ...
              return (
                <Grid key={e.codigo} size={{ xs: 12, md: 6 }}>
                  <Card sx={{ bgcolor: '#111827', border: `1px solid ${alpha(tc, 0.22)}`, borderRadius: 2 }}>
                    <CardContent sx={{ p: '16px !important' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
                        <Box>
                          <Typography sx={{ fontSize: 10, fontFamily: 'monospace', color: tc, mb: 0.25 }}>{e.codigo}</Typography>
                          <Typography sx={{ fontWeight: 700, color: '#FFF', fontSize: 14, lineHeight: 1.3 }}>{e.nombre}</Typography>
                          <Chip label={e.tipo.replace(/_/g, ' ')} size="small" sx={{ fontSize: 9, height: 18, bgcolor: alpha(tc, 0.15), color: tc, mt: 0.5 }} />
                        </Box>
                        <Chip label={e.estado} size="small" sx={{ fontSize: 9, height: 18, bgcolor: e.estado === 'activa' ? alpha(QMS_COLOR, 0.15) : 'rgba(255,255,255,0.06)', color: e.estado === 'activa' ? QMS_COLOR : 'rgba(255,255,255,0.4)' }} />
                      </Box>
                      <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#D97706' }}>{e.promedio}</Typography>
                          <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>CSAT</Typography>
                        </Box>
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography sx={{ fontSize: 20, fontWeight: 800, color: e.nps >= 60 ? QMS_COLOR : '#D97706' }}>{e.nps}</Typography>
                          <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>NPS</Typography>
                        </Box>
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#0369A1' }}>{e.respuestas}</Typography>
                          <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>Respuestas</Typography>
                        </Box>
                        <Box sx={{ flex: 1, display: 'flex', alignItems: 'center' }}>
                          <Box sx={{ width: '100%' }}>
                            <Box sx={{ height: 6, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.06)', mb: 0.5 }}>
                              <Box sx={{ width: `${(e.promedio / 5) * 100}%`, height: '100%', borderRadius: 3, bgcolor: '#D97706' }} />
                            </Box>
                            <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.3)' }}>de 5.0 estrellas</Typography>
                          </Box>
                        </Box>
                      </Box>
                    </CardContent>
        ...
        <TabPanel value={tab} index={1}>
          <Card sx={{ bgcolor: '#111827', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 2 }}>
            <CardContent>
              <Typography sx={{ fontWeight: 700, color: '#FFF', mb: 2, fontSize: 14 }}>Análisis por pregunta — Satisfacción Clientes Q2 2026</Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {PREGUNTAS.map((p, i) => (
                  <Box key={i} sx={{ pb: 2, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography sx={{ fontSize: 13, color: '#FFF', flex: 1 }}>{p.pregunta}</Typography>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 2 }}>
                        ...
                        <Typography sx={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>/ 5</Typography>
                      </Box>
                    </Box>
                    <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'flex-end', height: 36 }}>
                      {p.dist.map((v, j) => (
                        <Box key={j} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.25 }}>
                          ...
                          <Typography sx={{ fontSize: 8, color: 'rgba(255,255,255,0.3)' }}>{'★'.repeat(j + 1)}</Typography>
                        </Box>
                      ))}
                    </Box>
                  </Box>
        ...
        <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#1F2937', color: '#FFF' } }}>
          ...
          <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
            <TextField label="Nombre de la Encuesta" fullWidth size="small" InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }} sx={{ '& .MuiOutlinedInput-root': { color: '#FFF', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }} />
            ...
            <TextField label="Descripción / Objetivo" multiline rows={2} fullWidth size="small" InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }} sx={{ '& .MuiOutlinedInput-root': { color: '#FFF', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }} />
            <TextField label="Fecha de Cierre" type="date" fullWidth size="small" InputLabelProps={{ shrink: true, sx: { color: 'rgba(255,255,255,0.5)' } }} sx={{ '& .MuiOutlinedInput-root': { color: '#FFF', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }} />
          </DialogContent>
