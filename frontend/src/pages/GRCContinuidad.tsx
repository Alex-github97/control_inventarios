      ...
      <Box sx={{ p:3, background:PAGE_BG, minHeight:'100vh' }}>
        <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:3 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
            ...
            <Box><Typography variant="h5" sx={{ fontWeight:800, color:'#FFF', lineHeight:1 }}>Planes de Continuidad</Typography><Typography sx={{ fontSize:12, color:LBL }}>GRC · BCP, DRP, ERP y planes de crisis</Typography></Box>
        ...
        <Box sx={{ display:'flex', gap:2 }}>
          <Box sx={{ flex:1, minWidth:0 }}>
            <Paper sx={{ bgcolor:'transparent', overflowX:'auto' }}>
              <Table size="small">
                ...
                <TableBody>
                  {filtered.map(p=>(
                    <TableRow key={p.id} onClick={()=>setSel(p)} sx={{ cursor:'pointer', bgcolor:sel?.id===p.id?alpha(GRC_COLOR,.06):'transparent', '&:hover':{ bgcolor:alpha(GRC_COLOR,.04) }, '& td':{ borderColor:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.8)', fontSize:12 } }}>
                      <TableCell sx={{ fontSize:10.5, color:LBL }}>{p.id}</TableCell>
                      <TableCell sx={{ maxWidth:200 }}><Typography sx={{ fontSize:12.5, color:'#FFF', fontWeight:600, lineHeight:1.3 }}>{p.nombre}</Typography></TableCell>
          ...
          {sel&&(
            <Box sx={{ width:370, flexShrink:0, bgcolor:CARD_BG, border:`1px solid ${CARD_BOR}`, borderRadius:2, p:2.5, height:'fit-content' }}>
              ...
              <Typography sx={{ fontSize:11.5, color:'rgba(255,255,255,0.65)', mb:.5 }}>{sel.ultimoPrueba}</Typography>
              <Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.5)', lineHeight:1.6, mb:2 }}>{sel.resultadoPrueba}</Typography>
        ...
        <Dialog open={dlgOpen} onClose={()=>setDlgOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx:{ bgcolor:'#1F2937', color:'#FFF' } }}>
          ...
          <DialogContent sx={{ display:'flex', flexDirection:'column', gap:2, pt:'16px !important' }}>
            ...
            <Box sx={{ display:'flex', gap:2 }}>
              ...
              <TextField label="Fecha Aprobación" type="date" fullWidth size="small" value={form.fechaAprobacion} onChange={sf('fechaAprobacion')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
            </Box>
            <TextField label="Fecha Revisión" type="date" fullWidth size="small" value={form.fechaRevision} onChange={sf('fechaRevision')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
