...
import { Box, Typography, Grid, Card, CardContent, Chip, alpha, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from '@mui/material'
      ...
      <Box sx={{ p: 3, background: PAGE_BG, minHeight: '100vh' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 800, color: '#FFF', lineHeight: 1 }}>Plan de Emergencias</Typography>
              <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Brigada, planes de respuesta y simulacros · Resolución 0312/2019</Typography>
            </Box>
        ...
        <Grid container spacing={2.5}>
            ...
            <Card sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2 }}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  ...
                  <Typography sx={{ fontWeight: 700, color: '#fff', fontSize: 15 }}>Brigada de Emergencias</Typography>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {BRIGADA.map((m, i) => (
                    <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1 }}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#fff', lineHeight: 1.2 }}>{m.nombre}</Typography>
                        <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{m.rol}</Typography>
                      </Box>
                      {m.certificado
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            {/* Planes */}
            <Card sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, mb: 2.5 }}>
              <CardContent>
                <Typography sx={{ fontWeight: 700, color: '#fff', fontSize: 15, mb: 2 }}>Planes de Respuesta</Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {PLANES.map((p, i) => (
                    <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1 }}>
                      <Box sx={{ flex: 1 }}>
                        <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#fff', lineHeight: 1.2 }}>{p.nombre}</Typography>
                        <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Rev: {p.proxima_revision} · {p.version}</Typography>
                      </Box>
            ...
            <Card sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2 }}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                  ...
                  <Typography sx={{ fontWeight: 700, color: '#fff', fontSize: 15 }}>Simulacros realizados</Typography>
                </Box>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {simulacros.map((s, i) => (
                    <Box key={i} sx={{ p: '10px 12px', bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 1, borderLeft: `3px solid ${s.resultado === 'Satisfactorio' ? '#22c55e' : '#f59e0b'}` }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.25 }}>
                        <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>{s.tipo}</Typography>
                        <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{s.fecha}</Typography>
                      </Box>
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        <Chip label={`${s.participantes} participantes`} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.4)', fontSize: 10 }} />
        ...
        {/* Dialog simulacro */}
        <Dialog open={openSim} onClose={() => setOpenSim(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#0A1628', color: '#fff' } }}>
          <DialogTitle sx={{ borderBottom: '1px solid rgba(255,255,255,0.08)', fontWeight: 700 }}>Registrar Simulacro</DialogTitle>
          ...
          <DialogActions sx={{ p: 2, gap: 1, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            <Button onClick={() => setOpenSim(false)} sx={{ color: 'rgba(255,255,255,0.5)' }}>Cancelar</Button>
