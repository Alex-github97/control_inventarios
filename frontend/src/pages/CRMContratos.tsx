    ...
    <Layout>
      <Box sx={{ p: 3, bgcolor: DARK_BG, minHeight: '100vh' }}>
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#FFF' }}>Contratos + SLA</Typography>
                  ...
                  {CONTRATOS.map((c, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: urgente ? alpha(CRM_COLOR, 0.04) : undefined }}>
                        ...
                        <td style={{ padding: '10px 14px', fontSize: 12, color: 'rgba(255,255,255,0.5)', whiteSpace: 'nowrap' }}>{c.servicio}</td>
                        <td style={{ padding: '10px 14px' }}>
                          <Chip label={c.estado} size="small" sx={{ bgcolor: alpha(col, 0.15), color: col, fontSize: 9.5, fontWeight: 700 }} />
                        </td>
                        <td style={{ padding: '10px 14px', fontSize: 11.5, color: 'rgba(255,255,255,0.4)', whiteSpace: 'nowrap' }}>{c.inicio}</td>
                        <td style={{ padding: '10px 14px', fontSize: 11.5, color: urgente ? CRM_COLOR : 'rgba(255,255,255,0.4)', fontWeight: urgente ? 700 : 400, whiteSpace: 'nowrap' }}>{c.fin}</td>
                        ...
                        <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                          <Chip label={c.auto_renovacion ? 'Sí' : 'No'} size="small"
                            sx={{ bgcolor: c.auto_renovacion ? alpha('#059669', 0.15) : 'rgba(255,255,255,0.06)', color: c.auto_renovacion ? '#059669' : 'rgba(255,255,255,0.4)', fontSize: 9.5 }} />
                        </td>
        ...
        {tab === 1 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 0.5 }}>SLAs — Almacenes Éxito S.A. (CON-2026-012)</Typography>
            <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', mb: 2 }}>Medición mensual · Datos al 2026-06-01</Typography>
              ...
              return (
                <Box key={i} sx={{ mb: 2.5 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75 }}>
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{s.indicador}</Typography>
                    <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                      <Typography sx={{ fontSize: 11.5, color: 'rgba(255,255,255,0.4)' }}>Meta: {s.objetivo}{s.unidad}</Typography>
        ...
        {tab === 2 && (
          <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, p: 2.5 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF', mb: 2 }}>Calendario de Vencimientos</Typography>
              ...
              {CONTRATOS.filter(c => c.estado === 'ACTIVO').sort((a, b) => a.dias_vence - b.dias_vence).map((c, i) => {
                ...
                return (
                  <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1.5, bgcolor: alpha(col, 0.06), border: `1px solid ${alpha(col, 0.2)}`, borderRadius: 1.5 }}>
                    <Box>
                      <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{c.cliente}</Typography>
                      <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{c.codigo} · {c.servicio}</Typography>
                    </Box>
                    <Box sx={{ textAlign: 'right' }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 800, color: col }}>{c.dias_vence} días</Typography>
                      <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>{c.fin}</Typography>
                    </Box>
                  </Box>
                )
              })}
