      ...
      <Box sx={{ p:3, background:PAGE_BG, minHeight:'100vh' }}>
        <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:3 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
            ...
            <Box><Typography variant="h5" sx={{ fontWeight:800, color:'#FFF', lineHeight:1 }}>Gestión de Incidentes</Typography><Typography sx={{ fontSize:12, color:LBL }}>GRC · Registro, investigación y cierre de incidentes</Typography></Box>
        ...
        <Box sx={{ display:'flex', gap:2 }}>
          <Box sx={{ flex:1, minWidth:0 }}>
            <Paper sx={{ bgcolor:'transparent', overflowX:'auto' }}>
              <Table size="small">
                ...
                <TableBody>
                  {filtered.map(i=>(
                    <TableRow key={i.id} onClick={()=>setSel(i)} sx={{ cursor:'pointer', bgcolor:sel?.id===i.id?alpha(GRC_COLOR,.06):'transparent', '&:hover':{ bgcolor:alpha(GRC_COLOR,.04) }, '& td':{ borderColor:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.8)', fontSize:12 } }}>
                      <TableCell sx={{ fontSize:10.5, color:LBL }}>{i.id}</TableCell>
                      <TableCell sx={{ maxWidth:220 }}><Typography sx={{ fontSize:12.5, color:'#FFF', fontWeight:600, lineHeight:1.3 }}>{i.titulo}</Typography></TableCell>
                      ...
                      <TableCell sx={{ fontSize:11, color:'rgba(255,255,255,0.5)' }}>{i.fechaCierre||'—'}</TableCell>
                      <TableCell>
          ...
          {sel&&(
            <Box sx={{ width:370, flexShrink:0, bgcolor:'#FFFFFF', border:`1px solid #E5E7EB`, borderRadius:2, p:2.5, height:'fit-content' }}>
              ...
              {sel.acciones.map((a,i)=><Box key={i} sx={{ display:'flex', gap:1, mb:.75, alignItems:'flex-start', p:.75, bgcolor:'rgba(255,255,255,0.03)', borderRadius:1 }}><Box sx={{ width:5, height:5, borderRadius:'50%', bgcolor:GRC_COLOR, mt:.75, flexShrink:0 }} /><Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.65)', lineHeight:1.5 }}>{a}</Typography></Box>)}
              <Divider sx={{ borderColor:'rgba(255,255,255,0.06)', my:1.5 }} />
        ...
        {/* Dialog acción */}
        <Dialog open={accionDlg} onClose={()=>setAccionDlg(false)} maxWidth="xs" fullWidth PaperProps={{ sx:{ bgcolor:'#1F2937', color:'#FFF' } }}>
          <DialogTitle sx={{ fontWeight:700, fontSize:15 }}>Registrar Acción</DialogTitle>
          <DialogContent sx={{ pt:'12px !important' }}><TextField label="Descripción de la acción tomada" multiline rows={3} fullWidth size="small" value={nuevaAccion} onChange={e=>setNuevaAccion(e.target.value)} InputLabelProps={ILB} sx={TF} /></DialogContent>
          <DialogActions sx={{ px:3, pb:2 }}><Button onClick={()=>setAccionDlg(false)} sx={{ color:'rgba(255,255,255,0.5)' }}>Cancelar</Button><Button variant="contained" onClick={addAccion} sx={{ bgcolor:GRC_COLOR, '&:hover':{ bgcolor:'#5B21B6' } }}>Registrar</Button></DialogActions>
        </Dialog>
        ...
        <Dialog open={dlgOpen} onClose={()=>setDlgOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx:{ bgcolor:'#1F2937', color:'#FFF' } }}>
          ...
          <DialogContent sx={{ display:'flex', flexDirection:'column', gap:2, pt:'16px !important' }}>
            <TextField label="Título del Incidente" fullWidth size="small" value={form.titulo} onChange={sf('titulo')} InputLabelProps={ILB} sx={TF} />
            <Box sx={{ display:'flex', gap:2 }}>
              ...
              <FormControl size="small" fullWidth><InputLabel sx={{ color:'rgba(255,255,255,0.5)' }}>Severidad</InputLabel><Select label="Severidad" value={form.severidad} onChange={e=>ss('severidad',e.target.value)} sx={{ color:'#FFF', '& fieldset':{ borderColor:'rgba(255,255,255,0.15)' } }}>{SEVERIDADES.map(s=><MenuItem key={s} value={s}>{s}</MenuItem>)}</Select></FormControl>
            </Box>
            <Box sx={{ display:'flex', gap:2 }}>
            ...
            <Box sx={{ display:'flex', gap:2 }}>
              <TextField label="Fecha Detección" type="date" fullWidth size="small" value={form.fechaDeteccion} onChange={sf('fechaDeteccion')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
              <TextField label="Fecha Cierre" type="date" fullWidth size="small" value={form.fechaCierre} onChange={sf('fechaCierre')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
            </Box>
          ...
          <DialogActions sx={{ px:3, pb:2 }}><Button onClick={()=>setDlgOpen(false)} sx={{ color:'rgba(255,255,255,0.5)' }}>Cancelar</Button><Button variant="contained" onClick={handleSave} sx={{ bgcolor:GRC_COLOR, '&:hover':{ bgcolor:'#5B21B6' } }}>{editId?'Guardar Cambios':'Registrar'}</Button></DialogActions>
        </Dialog>
