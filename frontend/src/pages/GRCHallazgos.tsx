      ...
      <Box sx={{ p:3, background:PAGE_BG, minHeight:'100vh' }}>
        <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:3 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
            ...
            <Box><Typography variant="h5" sx={{ fontWeight:800, color:'#FFF', lineHeight:1 }}>Gestión de Hallazgos</Typography><Typography sx={{ fontSize:12, color:LBL }}>GRC · Seguimiento de hallazgos y planes de acción</Typography></Box>
        ...
        <Box sx={{ display:'flex', gap:2 }}>
          <Box sx={{ flex:1, minWidth:0 }}>
            <Paper sx={{ bgcolor:'transparent', overflowX:'auto' }}>
              <Table size="small">
                ...
                <TableBody>
                  {filtered.map(h=>(
                    <TableRow key={h.id} onClick={()=>setSel(h)} sx={{ cursor:'pointer', bgcolor:sel?.id===h.id?alpha(GRC_COLOR,.06):'transparent', '&:hover':{ bgcolor:alpha(GRC_COLOR,.04) }, '& td':{ borderColor:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.8)', fontSize:12 } }}>
                      <TableCell sx={{ fontSize:10.5, color:LBL }}>{h.id}</TableCell>
                      <TableCell sx={{ maxWidth:220 }}><Typography sx={{ fontSize:12.5, color:'#FFF', fontWeight:600, lineHeight:1.3 }}>{h.titulo}</Typography></TableCell>
                      ...
                      <TableCell sx={{ fontSize:11, color:h.estado!=='CERRADO'&&h.fechaLimite<'2026-06-21'?'#DC2626':'rgba(255,255,255,0.7)' }}>{h.fechaLimite}</TableCell>
                      <TableCell>
          ...
          {sel&&(
            <Box sx={{ width:370, flexShrink:0, bgcolor:'#FFFFFF', border:`1px solid #E5E7EB`, borderRadius:2, p:2.5, height:'fit-content' }}>
              ...
              <Typography sx={{ fontSize:12, color:'rgba(255,255,255,0.7)', lineHeight:1.7, mb:2 }}>{sel.planAccion}</Typography>
              <Divider sx={{ borderColor:'rgba(255,255,255,0.06)', my:1.5 }} />
              ...
              {sel.evidencias.map((ev,i)=>(
                <Box key={i} sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', p:.75, mb:.5, bgcolor:'rgba(255,255,255,0.03)', borderRadius:1, border:'1px solid rgba(255,255,255,0.06)' }}>
                  <Typography sx={{ fontSize:11, color:'rgba(255,255,255,0.7)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1, mr:1 }}>{ev}</Typography>
        ...
        <Dialog open={dlgOpen} onClose={()=>setDlgOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx:{ bgcolor:'#1F2937', color:'#FFF' } }}>
          ...
          <DialogContent sx={{ display:'flex', flexDirection:'column', gap:2, pt:'16px !important' }}>
            <TextField label="Título del Hallazgo" fullWidth size="small" value={form.titulo} onChange={sf('titulo')} InputLabelProps={ILB} sx={TF} />
            <Box sx={{ display:'flex', gap:2 }}>
            ...
            <Box sx={{ display:'flex', gap:2 }}>
              <TextField label="Fuente / Origen" fullWidth size="small" value={form.fuente} onChange={sf('fuente')} InputLabelProps={ILB} sx={TF} />
              <FormControl size="small" fullWidth><InputLabel sx={{ color:'rgba(255,255,255,0.5)' }}>Categoría</InputLabel><Select label="Categoría" value={form.categoria} onChange={e=>ss('categoria',e.target.value)} sx={{ color:'#FFF', '& fieldset':{ borderColor:'rgba(255,255,255,0.15)' } }}>{CATEGORIAS.map(c=><MenuItem key={c} value={c}>{c}</MenuItem>)}</Select></FormControl>
            </Box>
            <Box sx={{ display:'flex', gap:2 }}>
            ...
            <Box sx={{ display:'flex', gap:2 }}>
              ...
              <TextField label="Fecha Límite" type="date" fullWidth size="small" value={form.fechaLimite} onChange={sf('fechaLimite')} InputLabelProps={{ shrink:true, sx:{ color:'rgba(255,255,255,0.5)' } }} sx={TF} />
            </Box>
            ...
            <TextField label="Plan de Acción" multiline rows={2} fullWidth size="small" value={form.planAccion} onChange={sf('planAccion')} InputLabelProps={ILB} sx={TF} />
          </DialogContent>
