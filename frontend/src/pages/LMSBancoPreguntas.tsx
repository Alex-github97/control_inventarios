    ...
    <Layout>
      <Box sx={{ p: 3, bgcolor: DARK_BG, minHeight: '100vh' }}>
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#FFF' }}>Banco de Preguntas</Typography>
          ...
          {porTipo.map((t, i) => {
            ...
            return (
              <Grid key={i} size={{ xs: 6, md: 3 }}>
                <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${alpha(col, 0.25)}`, borderRadius: 2, p: 2 }}>
                  <Typography sx={{ fontSize: 22, fontWeight: 800, color: '#FFF', lineHeight: 1 }}>{t.count}</Typography>
        ...
        {/* Filtros */}
        <Box sx={{ display: 'flex', gap: 1, mb: 1.5, bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, px: 2, py: 1, alignItems: 'center' }}>
          <Search sx={{ color: 'rgba(255,255,255,0.3)', fontSize: 20 }} />
          <InputBase
            placeholder="Buscar pregunta..."
            value={busqueda}
            onChange={e => setBusqueda(e.target.value)}
            sx={{ flex: 1, color: '#FFF', fontSize: 13.5 }}
          />
        </Box>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 3 }}>
          {NIVELES.map(n => {
            const col = DIFICULTAD_COLORS[n] || 'rgba(255,255,255,0.4)'
            return (
              <Chip key={n} label={n} size="small" onClick={() => setNivel(n)}
                sx={{ cursor: 'pointer', bgcolor: nivel === n ? alpha(col, 0.2) : 'transparent', color: nivel === n ? col : 'rgba(255,255,255,0.35)', border: `1px solid ${nivel === n ? alpha(col, 0.4) : 'rgba(255,255,255,0.08)'}`, fontWeight: nivel === n ? 700 : 400 }} />
            )
          })}
          {TIPOS.map(t => {
            ...
            return (
              <Chip key={t} label={t === 'Todos' ? 'Todos los tipos' : t.charAt(0) + t.slice(1).toLowerCase().replace('_', ' ')} size="small" onClick={() => setTipo(t)}
                sx={{ cursor: 'pointer', bgcolor: tipo === t ? alpha(col, 0.2) : 'transparent', color: tipo === t ? col : 'rgba(255,255,255,0.35)', border: `1px solid ${tipo === t ? alpha(col, 0.4) : 'rgba(255,255,255,0.08)'}`, fontWeight: tipo === t ? 700 : 400 }} />
            )
          })}
        </Box>
        ...
        <Box sx={{ bgcolor: CARD_BG, border: `1px solid ${CARD_BOR}`, borderRadius: 2, overflow: 'hidden' }}>
          <Box sx={{ p: 2, borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#FFF' }}>Preguntas ({filtradas.length})</Typography>
          </Box>
                ...
                {filtradas.map((p, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td style={{ padding: '10px 14px', fontSize: 12, color: 'rgba(255,255,255,0.55)', whiteSpace: 'nowrap' }}>{p.categoria}</td>
                      <td style={{ padding: '10px 14px', fontSize: 12.5, color: 'rgba(255,255,255,0.75)', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.enunciado}</td>
