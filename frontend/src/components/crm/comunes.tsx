/**
 * Lo que repiten las quince pantallas del CRM.
 *
 * Existe para que la respuesta a «¿y si no hay datos?» sea la misma en todas.
 * Cuando cada pantalla resuelve el vacío a su manera, el módulo se siente como
 * quince programas distintos: una dice «Sin resultados», otra deja la tabla en
 * blanco, otra muestra un cero que parece un dato real.
 */
...
export const CRM_COLOR = COLOR_MODULO
  ...
  if (cargando) {
    return (
      <Box sx={{ py: 6, textAlign: 'center' }}>
        <CircularProgress size={28} sx={{ color: CRM_COLOR }} />
      </Box>
    )
  }
  if (error) {
...
export const COLOR_SEGMENTO: Record<string, string> = {
  CORPORATIVO: CRM_COLOR, ESTRATEGICO: '#7C3AED',
